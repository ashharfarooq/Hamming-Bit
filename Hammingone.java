public class Hamming {
    ...
    public static void main(String[] args) {
       ...
       /* for (int i = 1; i < msglength; i *= 2) {   // i = 1, 2, 4, 8, ...
    int set = 0;

    // Inner loop: take 'i' bits, skip 'i' bits
    for (int t = i - 1; t < msglength; t += 2 * i) {
        for (int j = 0; j < i && (t + j) < msglength; j++) {
            set += msg[t + j];
        }
    }

    System.out.println("For h" + i + " → Sum = " + set);
}
        }*/
